...
echo "Then open http://localhost:3000 to view traces in Grafana"
sleep 10

echo -e "\nQuerying Tempo for recent traces..."
curl -s "http://localhost:3200/api/search?limit=10" | jq '.' || echo "Could not query Tempo (jq may not be installed)"
